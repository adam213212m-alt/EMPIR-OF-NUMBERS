...
# --- تعريف جميع قوالب HTML بالكامل أولاً ---
...
# --- مسارات الألعاب والمحاسبة الكاملة ---
    ...
    if request.method == 'POST':
        ...
        if action_type == 'book' and draw_state.status == 'idle':
            ...
            if user.balance >= 20.0 and not GoldenNumberBooking.query.filter_by(number=number).first():
                ...
                return jsonify({"success": False, "msg": "رصيد غير كافي أو الرقم محجوز!"})
...
def admin_accounting():
    ...
    return render_template_string(ADMIN_ACCOUNTING_TEMPLATE, t=t, vault_balance=vault.vault_balance, logs=logs, users_list=User.query.all(), msg=msg)
