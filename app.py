    ...
    if request.method == 'POST':
        ...
        if action == 'open_box':
            ...
            if clicked_box_idx is not None:
                ...
                cursor.execute("SELECT balance FROM users WHERE username=?", (username,))
                bal = cursor.fetchone()[0]
                cost = 1.0  # خصم الدولار عند أول نقرة
                ...
                if bal >= cost:
                    ...
                    # فحص التطابق أو الوصول للمحاولة 36 عالمياً
                    if total_att >= 36 or (total_att % 36 == 0):
                        is_win = True
                        total_att = 0
                    else:
                        is_win = (revealed_val == 3 or total_att % 5 == 0) # نسبة فوز مدروسة مع النقرة الأولى
        ...
        if action == 'play_balloon':
            ...
            if bal >= cost:
                ...
                if attempts_since >= current_target:
                    ...
                    balloon_color = "#22c55e" # أخضر عند الفوز وبقاء البالون منتفخاً
                else:
                    is_win = False
                    is_popped = True
...
<body>
    ...
    <div class="boxes-container">
        ...
        <p style="color: #cbd5e1; font-size: 15px;">الخصم والفحص يتمان فوراً عند النقرة الأولى</p>
        ...
        <form method="POST" id="gameForm">
            ...
            <input type="hidden" name="box_index" id="boxIndexInput">
            <div class="boxes-grid">
                {% for i in range(15) %}
                    <div class="box-card {% if clicked_box_idx == i %}selected{% endif %}" id="box-{{ i }}" onclick="openBox({{ i }})">
                        <span id="box-icon-{{ i }}">{% if clicked_box_idx == i %}🔓{% else %}📦{% endif %}</span>
                        <span id="box-text-{{ i }}" style="font-size: 12px; margin-top: 5px; {% if clicked_box_idx == i %}display:none;{% endif %}">صندوق {{ i+1 }}</span>
                        <span id="box-val-{{ i }}" style="{% if clicked_box_idx != i %}display:none;{% endif %} color: #ffd700; font-size: 22px; margin-top: 4px;">{% if clicked_box_idx == i %}{{ revealed_val }}{% else %}{{ boxes[i] }}{% endif %}</span>
                        {% if clicked_box_idx == i and result_text %}
    ...
    <script>
        function openBox(index) {
            document.getElementById('boxIndexInput').value = index;
