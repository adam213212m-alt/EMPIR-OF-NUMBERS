...
def init_db():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS lotto_state (
            id INTEGER PRIMARY KEY,
            winner_num INTEGER DEFAULT 0,
            status TEXT DEFAULT 'idle'
        )
    ''')
    cursor.execute('SELECT COUNT(*) FROM lotto_state')
    if cursor.fetchone()[0] == 0:
        cursor.execute('INSERT INTO lotto_state (id, winner_num, status) VALUES (1, 0, "idle")')

    cursor.execute('''
        CREATE TABLE IF NOT EXISTS lotto_bookings (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            username TEXT,
            number INTEGER
        )
...
def game_golden_boxes():
    ...
    if request.method == 'POST':
        ...
        if action == 'open_box':
            clicked_box_idx = request.form.get('box_index')
            if clicked_box_idx is not None:
                clicked_box_idx = int(clicked_box_idx)
                cursor.execute("SELECT balance FROM users WHERE username=?", (username,))
                bal = cursor.fetchone()[0]
                cost = 1.0
                ...
                if bal >= cost:
                    ...
                    total_att += 1
                    revealed_val = boxes[clicked_box_idx]
                    
                    if total_att >= 36 or (total_att % 36 == 0):
                        is_win = True
                        total_att = 0
                    else:
                        is_win = (revealed_val == 3 or total_att % 5 == 0)
                    ...
                    if is_win:
                        ...
                        conn.commit()
                        result_text = f"WIN - مبروك لقد فزت بـ 20$ (الرقم المكشوف: {revealed_val})"
                    else:
                        conn.commit()
                        result_text = f"LOST - حظ أوفر (الرقم المكشوف: {revealed_val})"
                else:
    ...
    conn.close()
    return render_template_string(GAME_GOLDEN_BOXES_PAGE, username=username, balance=balance, total_att=total_att, boxes=boxes, result_text=result_text, revealed_val=revealed_val, clicked_box_idx=clicked_box_idx, msg=msg)
        ...
        if action == 'play_balloon':
            ...
            if bal >= cost:
                ...
                if attempts_since >= current_target:
                    ...
                    balloon_color = "#22c55e"
                else:
                    is_win = False
                    is_popped = True
                    balloon_color = random.choice(["#ffeb3b", "#ff9800", "#e91e63"])
...
@app.route('/game_wheel_of_fortune', methods=['GET', 'POST'])
def game_wheel_of_fortune():
    if 'username' not in session:
        return redirect(url_for('login'))
    ...
    if request.method == 'POST':
        ...
        if action == 'book_lotto':
            num = int(request.form.get('number', 0))
            cursor.execute("SELECT balance FROM users WHERE username=?", (username,))
            bal = cursor.fetchone()[0]
            cost = 50.0 # تكلفة حجز الرقم 50$
            
            if bal >= cost:
                cursor.execute("UPDATE users SET balance = balance - ? WHERE username=?", (cost, username))
                cursor.execute("UPDATE system_vault SET vault_balance = vault_balance + ? WHERE id=1", (cost,))
                cursor.execute("INSERT INTO lotto_bookings (username, number) VALUES (?, ?)", (username, num))
                conn.commit()
                lotto_msg = f"تم حجز الرقم {num} بنجاح مقابل 50$!"
            else:
                lotto_msg = "رصيدك غير كافٍ لحجز الرقم (التكلفة 50$)!"

        elif action == 'admin_draw_lotto' and username == 'admin1':
            forced_winner = int(request.form.get('forced_winner_num', 0))
            # اختيار الرقم الفائز (إما مفروض من المؤسس أو عشوائي من 1 إلى 5)
            winning_num = forced_winner if 1 <= forced_winner <= 5 else random.randint(1, 5)
            
            cursor.execute("UPDATE lotto_state SET winner_num=?, status='finished' WHERE id=1", (winning_num,))
            
            # التحقق ممن حجز الرقم الفائز لدفع الجائزة 200$
            cursor.execute("SELECT username FROM lotto_bookings WHERE number=?", (winning_num,))
            winners = cursor.fetchall()
            
            for w in winners:
                winner_user = w[0]
                cursor.execute("UPDATE users SET balance = balance + 200.0 WHERE username=?", (winner_user,))
                cursor.execute("UPDATE system_vault SET vault_balance = vault_balance - 200.0 WHERE id=1")
                cursor.execute("INSERT INTO financial_logs (action_type, admin_name, target_user, amount, log_time) VALUES ('جائزة لعبة الأرقام', 'admin1', ?, 200.0, ?)", 
                               (winner_user, time.strftime('%Y-%m-%d %H:%M')))

            conn.commit()
            lotto_msg = f"تم السحب! الرقم الفائز هو {winning_num} وتم توزيع الجوائز."

        elif action == 'reset_lotto' and username == 'admin1':
            cursor.execute("DELETE FROM lotto_bookings")
            cursor.execute("UPDATE lotto_state SET winner_num=0, status='idle' WHERE id=1")
            conn.commit()
            lotto_msg = "تمت إعادة ضبط اللعبة وتصفير الحجوزات بنجاح."

    cursor.execute("SELECT balance FROM users WHERE username=?", (username,))
    balance = cursor.fetchone()[0]
    
    cursor.execute("SELECT number, username FROM lotto_bookings")
    all_bookings = cursor.fetchall()
    user_booked_nums = [b[0] for b in all_bookings if b[1] == username]
    
    cursor.execute("SELECT winner_num, status FROM lotto_state WHERE id=1")
    lotto_state = cursor.fetchone()
    winner_num = lotto_state[0]
    lotto_status = lotto_state[1]

    conn.close()
    
    return render_template_string(GAME_WHEEL_PAGE, username=username, balance=balance, msg=msg, lotto_msg=lotto_msg, user_booked_nums=user_booked_nums, winner_num=winner_num, lotto_status=lotto_status)
...
def admin_accounting():
    ...
    cursor.execute("SELECT SUM(amount) FROM financial_logs WHERE action_type='جائزة لعبة الأرقام'")
    payout_res5 = cursor.fetchone()[0] or 0.0

    total_sales = sales_res
    total_payouts = payout_res1 + payout_res2 + payout_res3 + payout_res4 + payout_res5
...
<body>
    ...
    <div class="icons-grid">
        ...
        <a href="/game_wheel_of_fortune" class="icon-card"><div class="icon-logo">🎡</div><div class="icon-title">عجلة الثروة (الأرقام)</div></a>
...
<head>
    <meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>لعبة حجز الأرقام - ليرة</title>
    <style>
        ...
        .game-box { background: linear-gradient(135deg, #1f1a0f, #0d0d0d); border: 4px solid #ffd700; padding: 30px; border-radius: 20px; max-width: 500px; margin: 30px auto; box-shadow: 0 0 35px rgba(255,215,0,0.3); }
        .numbers-grid { display: flex; justify-content: center; gap: 10px; margin: 25px 0; }
        .num-btn { width: 65px; height: 65px; background: #ffffff; color: #000; font-size: 24px; font-weight: bold; border: none; border-radius: 12px; cursor: pointer; box-shadow: 0 4px 10px rgba(0,0,0,0.5); transition: 0.2s; }
        .num-btn:hover { transform: scale(1.05); }
        .num-btn.booked { background: #3b82f6 !important; color: white !important; }
        .num-btn.winning { background: #eab308 !important; color: black !important; transform: scale(1.15); box-shadow: 0 0 20px #eab308; }
        .winner-screen { font-size: 32px; font-weight: bold; background: #333; border: 3px solid #ffd700; width: 90px; height: 90px; display: flex; align-items: center; justify-content: center; margin: 15px auto; border-radius: 15px; color: #ffd700; }
        .back-btn { background: #3b82f6; color: white; text-decoration: none; padding: 8px 15px; border-radius: 6px; font-weight: bold; }
        .admin-secret-panel { background: #1f1f1f; border: 2px dashed #38bdf8; padding: 15px; border-radius: 10px; margin-top: 25px; text-align: right; }
        input[type="number"] { width: 100%; padding: 10px; margin: 5px 0 10px 0; border-radius: 6px; background: #252525; color: white; border: 1px solid #555; box-sizing: border-box; text-align: center; font-size: 16px; }
        .ctrl-btn { background: #22c55e; color: white; padding: 10px; border: none; border-radius: 6px; font-weight: bold; cursor: pointer; width: 100%; }
        .reset-btn { background: #ef4444; color: white; padding: 10px; border: none; border-radius: 6px; font-weight: bold; cursor: pointer; width: 100%; margin-top: 8px; }
    </style>
</head>
<body>
    <div class="header">
        <h2 style="color: #ffd700; margin: 0;">🔢 لعبة حجز الأرقام المحظوظة</h2>
    ...
    {% if lotto_msg %}<div style="background: #065f46; color: #34d399; padding: 12px; border-radius: 8px; margin-top: 15px; font-weight: bold;">{{ lotto_msg }}</div>{% endif %}

    <div class="game-box">
        <h3 style="color: #ffd700; margin-top: 0;">اختر رقمك المحظوظ</h3>
        <p style="color: #22c55e; font-size: 15px; font-weight: bold;">تكلفة حجز الرقم الواحد: 50$ (الجائزة 200$)</p>

        <div class="numbers-grid">
            {% for i in range(1, 6) %}
            <form method="POST" style="margin: 0;">
                <input type="hidden" name="action" value="book_lotto">
                <input type="hidden" name="number" value="{{ i }}">
                <button type="submit" class="num-btn {% if i in user_booked_nums %}booked{% endif %} {% if lotto_status == 'finished' and winner_num == i %}winning{% endif %}">
                    {{ i }}
                </button>
            </form>
            {% endfor %}
        ...
        <p style="color: #cbd5e1; font-size: 14px; margin-top: 15px;">الأرقام المحجوزة الخاصة بك: <b style="color: #38bdf8;">{% if user_booked_nums %}{{ user_booked_nums | join(', ') }}{% else %}لا توجد{% endif %}</b></p>

        <h4 style="color: #ffd700; margin-top: 25px;">الرقم الرابح:</h4>
        <div class="winner-screen" style="background: {% if lotto_status == 'finished' %}#eab308; color: black;{% else %}#333; color: #ffd700;{% endif %}">
            {% if lotto_status == 'finished' %}{{ winner_num }}{% else %}?{% endif %}
        </div>

        {% if lotto_status == 'finished' %}
            <div style="margin-top: 15px; font-size: 18px; font-weight: bold; color: {% if winner_num in user_booked_nums %}#34d399{% else %}#fca5a5{% endif %};">
                {% if winner_num in user_booked_nums %}
                    🎉 مبروك ربحت 200$ لأنك حجزت الرقم الفائز {{ winner_num }}!
                {% else %}
                    ❌ حظاً أوفر! الرقم الفائز هو {{ winner_num }} ولم تقم بحجزه.
                {% endif %}
            </div>
        {% endif %}

        {% if session.get('username') == 'admin1' %}
        <div class="admin-secret-panel">
            <h4 style="color: #38bdf8; margin-top: 0;">👑 لوحة تحكم المؤسس (سرية للغاية)</h4>
            <form method="POST">
                <input type="hidden" name="action" value="admin_draw_lotto">
                <label style="font-size: 13px; color: #cbd5e1;">اختر الرقم الفائز يدوياً (من 1 إلى 5):</label>
                <input type="number" name="forced_winner_num" min="1" max="5" placeholder="مثال: 3" required>
                <button type="submit" class="ctrl-btn">⚡ تشغيل السحب الفوري وتوزيع الجوائز</button>
            </form>
            <form method="POST">
                <input type="hidden" name="action" value="reset_lotto">
                <button type="submit" class="reset-btn">🔄 تصفير اللعبة وإعادة ضبط الحجوزات</button>
            </form>
        </div>
        {% endif %}
    </div>
